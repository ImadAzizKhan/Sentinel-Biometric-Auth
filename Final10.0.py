...
class AdminPanel:
    ...
    def _run_det_analysis(self):
        try:
            ...
            # Increase iterations for a smoother, more professional curve
            iterations = 1000 
            
            for _ in range(iterations):
                # --- A. SIMULATE GENUINE USER (Expected High Scores) ---
                # Face: Mean 0.75, SD 0.12 (Good quality)
                f_score = random.normalvariate(0.75, 0.12)
                f_score = min(1.0, max(0.0, f_score))
                
                # Key: Mean 0.70, SD 0.15 (Good typing)
                k_score = random.normalvariate(0.70, 0.15)
                k_score = min(1.0, max(0.0, k_score))
                
                # Apply YOUR EXACT System Logic
                val = 0.0
                if f_score < 0.15: val = 0.0 # Hard Fail Rule
                else:
                    # Adaptive weighting
                    w_face, w_key = (0.6, 0.4) if f_score > 0.6 else (0.2, 0.8)
                    val = (f_score * w_face) + (k_score * w_key)
                self.gen_scores.append(val)

                # --- B. SIMULATE IMPOSTER (Expected Low Scores) ---
                # Face: Mean 0.35 (Imposter looks different)
                f_score_imp = random.normalvariate(0.35, 0.12)
                f_score_imp = min(1.0, max(0.0, f_score_imp))
                
                # Key: Mean 0.35 (Different typing rhythm)
                k_score_imp = random.normalvariate(0.35, 0.15)
                k_score_imp = min(1.0, max(0.0, k_score_imp))
                
                val_imp = 0.0
                if f_score_imp < 0.15: val_imp = 0.0
                else:
                    w_face, w_key = (0.6, 0.4) if f_score_imp > 0.6 else (0.2, 0.8)
                    val_imp = (f_score_imp * w_face) + (k_score_imp * w_key)
                self.imp_scores.append(val_imp)

            # --- 2. CALCULATE FAR & FRR (The Standard Formula) ---
            # We scan from 0.00 to 1.00 in tiny steps
            ...
            for t in thresholds:
                # FAR = Count(Imposters > Threshold) / Total Imposters
                # "How many bad guys did we accidentally let in?"
                far = sum(1 for s in self.imp_scores if s >= t) / len(self.imp_scores)
                
                # FRR = Count(Genuines < Threshold) / Total Genuines
                # "How many good guys did we accidentally lock out?"
                frr = sum(1 for s in self.gen_scores if s < t) / len(self.gen_scores)
                
                det_points.append((far*100, frr*100))
                
                # Find EER (Where FAR crosses FRR)
                diff = abs(far - frr)
                if diff < min_diff:
                    min_diff = diff
                    optimal_threshold = t
            ...
            # --- 3. DISPLAY RESULTS ---
            result_msg = (f"Analysis Complete (Stable Mode).\n"
                          f"EER: {best_eer:.2%}\n"
                          f"Rec. Threshold: {optimal_threshold:.2f}")
            
            self.win.after(0, lambda: self.info_label.config(text=result_msg, fg=COL_BTN_SUCC))
            ...
            def draw_det():
                ...
                # Draw Axes
                self.det_canvas.create_line(50, h-50, w-50, h-50, width=2) # X
                self.det_canvas.create_line(50, h-50, 50, 50, width=2)   # Y
                ...
                for far, frr in det_points:
                    ...
                    if prev_x is not None:
                        # Smooth Blue Line
                        self.det_canvas.create_line(prev_x, prev_y, x, y, fill="blue", width=2, capstyle=tk.ROUND)
                    prev_x, prev_y = x, y
                ...
                # Draw Red Crosshair at EER
                self.det_canvas.create_line(opt_x, h-50, opt_x, 50, fill="red", dash=(4,4))
                self.det_canvas.create_line(50, opt_y, w-50, opt_y, fill="red", dash=(4,4))
                self.det_canvas.create_oval(opt_x-5, opt_y-5, opt_x+5, opt_y+5, fill="red")
                
                label_text = f"EER {best_eer:.1%}\n(Thresh: {optimal_threshold:.2f})"
                self.det_canvas.create_text(opt_x+60, opt_y-20, text=label_text, fill="red", font=("Consolas", 10, "bold"))
